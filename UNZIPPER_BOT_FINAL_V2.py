...
BOT_TAG          = "@F88UF9844"
AUTO_DEL_SEC     = 120           # auto-delete final status after N seconds
...
def bot_footer() -> str:
    return f"📢 {CHANNEL_LINK} | 👤 @{ADMIN_USERNAME}"
...
async def cb_policy(event):
    ...
    if event.sender_id != uid:
        await event.answer("❌ Not your button!", alert=True)
        return
    ...
    def _ydl():
        try:
            with yt_dlp.YoutubeDL({
                "outtmpl": ydl_out, "quiet": True, "no_warnings": True,
