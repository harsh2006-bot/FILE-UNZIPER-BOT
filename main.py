...
# ── Button sets ───────────────────────────────────────────
def stop_btn(fk):
    return [[Button.inline("⏹ Stop", data=f"stop:{fk}")]]

def output_btns(jid, ch=""):
    ch_label = f"📢 Send to {ch}" if ch else "📢 Send to Channel"
    return [
        [Button.inline("📲 Send here",  data=f"O:here:{jid}")],
        [Button.inline(ch_label,        data=f"O:ch:{jid}")],
        [Button.inline("❌ Cancel",     data=f"O:no:{jid}")],
    ]
...
async def cb_policy(ev):
    ...
    try:
        await ev.edit("✅ *Accepted!* Send /start",
            parse_mode="markdown",
...
@client.on(events.CallbackQuery(pattern=rb"O:"))
async def cb_output(ev):
    parts = ev.data.decode().split(":")
    mode  = parts[1]   # here / ch / no
    jid   = parts[2]
...
# ── Stop button ───────────────────────────────────────────
@client.on(events.CallbackQuery(pattern=rb"stop:"))
async def cb_stop(ev):
    fkey = ev.data.decode().split(":",1)[1]
    flag = JOBS.get(fkey)
    if not flag:
